...
const Home = () => {
  ...
  const { data: todos, isLoading: isFetchingTodos } = useQuery({
    queryKey: [`getTodos`, user.refetchData],
    queryFn: () =>
      getAllTodos({
        sort_order: "asc",
        category: "",
  ...
  useEffect(() => {
    if (!isFetchingTodos) {
      const tasks = todos?.data?.map((todo: { category: string; status: string }) => ({
        ...todo,
        done: todo.status === "completed" ? true : false,
        category: user.categories.filter((cat: Category) => cat.name === todo.category) ?? [],
      }));

      setUser({ ...user, tasks });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isFetchingTodos, user.refetchData]);
