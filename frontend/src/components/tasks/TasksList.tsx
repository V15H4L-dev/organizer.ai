...
import { calculateDateDifference, getFontColor, showToast, systemInfo } from "../../utils";
...
import { deleteTodo, markAsDone } from "../../services/api";
...
export const TasksList: React.FC = () => {
  ...
  const [selectedCatId, setSelectedCatId] = useState<UUID | undefined>(undefined);
  ...
  const confirmDeleteTask = async () => {
    ...
    if (selectedTaskId) {
      const response = await deleteTodo(selectedTaskId);
  ...
  const handleMarkSelectedAsDone = async () => {
    const response = await markAsDone({ ids: multipleSelectedTasks });
    if (response.status === 200) {
      ...
      setMultipleSelectedTasks([]);
      showToast(<div>Marked selected tasks as done.</div>);
    }
  };
            ...
            >
              ...
              <TaskInfo translate="no">
                ...
                <TaskHeader>
                  ...
                  <Tooltip title={`Confidence : ${task.confidence}`}>
                    <TaskDate>Sentiment : {task.sentiment}</TaskDate>
                  </Tooltip>
                </TaskHeader>
